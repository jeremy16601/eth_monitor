...
class EtherscanService extends Service {
    ...
    async saveAccount(eth,eth_balance,token_balance,transactions){
        const data=new this.ctx.model.Ethaccount();
        data.eth=eth;
    ...
    async updateAccount(id,eth_balance,token_balance,transactions){
        ...
        return this.ctx.model.Ethaccount.where({ _id: id }).update( update).exec();
    }
    ...
    async findByEth(eth){
        ...
        return this.ctx.model.Ethaccount.findOne(query, '_id', opts).exec();
    }
