  ...
  async index() {
    ...
    event.on('event_'+ctx.query.eth, async() => {
      while (true) {
        ...
        //查询是否存在
        let account = await service.etherscan.findByEth(eth);
        if (account == null) {
            account= await service.etherscan.saveAccount(eth, eth_balance, token_balance, transactions)
          console.log('save eth=' + JSON.stringify(account))
        } else {
          const eth2 = await service.etherscan.updateAccount(account._id, eth_balance, token_balance, transactions)
        ...
        for (let i = 1; i < table3[0].length; i++) {
          ...
          let isTx = await service.etherscan.findByTxHash(params.TxHash);
          if (isTx == null) {
            rs = await service.etherscan.saveEtherscans(params);
